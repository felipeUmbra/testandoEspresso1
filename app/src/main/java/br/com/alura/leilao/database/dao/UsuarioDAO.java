...
public class UsuarioDAO extends DatabaseHelper {
    ...
    public List<Usuario> todos() {
        ...
        List<Usuario> usuarios = armazenaUsuariosEmLista(c);
        c.close();
        return usuarios;
    }
    ...
    private Usuario preencheUsuario(Cursor c) {
        int idColumnIndex = c.getColumnIndex(UsuarioContrato._ID);
        int nomeColumnIndex = c.getColumnIndex(UsuarioContrato.CHAVE_NOME);
        ...
        if (nomeColumnIndex != -1) {
            nome = c.getString(nomeColumnIndex);
        }
